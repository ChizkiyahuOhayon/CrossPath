...
CROSSPATH_RUN="/root/autodl-tmp/weave/runs/CrossPath_FashionIQ_DQU_20260820_v1"
...
for CROSSPATH_CAT in dress shirt toptee; do
  ...
  "$CROSSPATH_PY" "$CROSSPATH_WEAVE/weave_extract_crosspath_dqu.py" \
    ...
    --split train \
    --output-dir "$CROSSPATH_INTERNAL/embeddings" \
    --batch-size 16 \
    > "$CROSSPATH_INTERNAL/extract.log" 2>&1
...
echo "CrossPath FashionIQ pipeline completed"
